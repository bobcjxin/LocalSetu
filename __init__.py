...
- -kkqyxp[keyword]：随机发送色图/男同图，其中keyword为可选参数，支持ID、@上传者、TAG模糊查询
- -上传se图[TAG][图片][TAG][图片][TAG][图片]，其中TAG为可选参数，可跟多张图片
- -上传se图[无参数]：进入上传模式，该模式下用户发送的所有图片均视为上传，发送[退出上传]或无操作20秒后自动退出
- -删除se图[ID]：删除指定ID色图，非审核组成员仅可删除本人上传的色图，他人色图将会推送至审核组
- -修改TAG[ID][TAG]：修改指定ID的自定义TAG
- -反和谐[ID]：色图被TX屏蔽时使用该指令，进行一次反和谐，后续发送色图均使用反和谐后文件
- -上传统计：让我康康谁才是LSP！
- -仓库地址：https://github.com/pcrbot/LocalSetu 有问题欢迎提issue
=======审核组用户有以下指令：
= =审核se图[上传][删除]：进入审核模式，每次发送待审核的色图，使用指令[保留][删除]后自动发送下一张，发送[退出审核]或20秒无操作自动退出
...
@sv.on_fullmatch(('se图帮助', 'setuhelp', 'setu帮助', 'LocalSetu'))
...
@sv.on_fullmatch(('审核se图上传', '审核se图删除'))
