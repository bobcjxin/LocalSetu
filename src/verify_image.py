    ...
    try:
        ...
        while VerifyImageProcess[user_id].time > 0 and VerifyImageProcess[user_id].state:
            if not VerifyImageProcess[user_id].sql_state:                      #没有待审核图则重新拿一张并发送
                ...
                # image = r'[CQ:image,file=file:///C:\Users\Administrator\Desktop\pcr\HoshinoBot\res\img\setu\10001.jpg]'
                image = str(MessageSegment.image(f'file:///{os.path.abspath(os.path.join(setu_folder,results[0]))}'))
